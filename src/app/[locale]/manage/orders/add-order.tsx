...
import { useAddGuestMutation } from "@/queries/useAccount";
...
export default function AddOrder() {
    ...
    const dishes: DishListResType["data"] = dishListQuery.data?.payload.data ?? [];
    ...
    const handleOrder = async () => {
        if (createOrdersMutation.isPending) return;
        try {
            let guestId = selectedGuest?.id;
            if (isNewGuest) {
                if (name && tableNumber) {
                    const addGuestRes = await addGuestMutation.mutateAsync({ name, tableNumber });
                    guestId = addGuestRes.payload.data.id;
                } else {
                    toast.error("Hãy Nhập tên khách và chọn số bàn để đặt món!");
                    return;
                }
            }
            ...
            const createOrdersRes = await createOrdersMutation.mutateAsync({ guestId, orders });
            toast(createOrdersRes.payload.message);
            reset();
        } catch (error) {}
    };
