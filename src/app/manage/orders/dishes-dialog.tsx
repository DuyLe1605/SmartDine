...
export function DishesDialog({ onChoose }: { onChoose: (dish: DishItem) => void }) {
    ...
    const getDishListQuery = useGetDishList();
    const data = getDishListQuery.data?.payload.data ?? [];
                                    ...
                                    {table.getRowModel().rows?.length ? (
                                        table.getRowModel().rows.map((row) => (
                                            <TableRow
                                                ...
                                                onClick={() => {
                                                    if (row.original.status === DishStatus.Available)
                                                        choose(row.original);
                                                }}
                                                className={cn("cursor-pointer", {
                                                    "cursor-not-allowed": row.original.status !== DishStatus.Available,
                                                })}
                                            >
